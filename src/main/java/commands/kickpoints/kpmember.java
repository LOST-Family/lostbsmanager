...
public class kpmember extends ListenerAdapter {
	...
	public void onSlashCommandInteraction(@Nonnull SlashCommandInteractionEvent event) {
		...
		if (c == null) {
			event.replyEmbeds(MessageUtil.buildEmbed(title, "Dieser Spieler existiert nicht oder ist in keinem Club.",
					MessageUtil.EmbedType.ERROR)).queue();
			return;
		}
			...
			public void run() {
				...
				if (c == null) {
					event.replyEmbeds(MessageUtil.buildEmbed(title,
							"Dieser Spieler existiert nicht oder ist in keinem Club.", MessageUtil.EmbedType.ERROR))
							.queue();
					return;
				}
