...
@SuppressWarnings("null") public class playerinfo extends ListenerAdapter {
		...
		new Thread(() -> {
			...
			if (conv == ConvertionType.ACCTOUSER) {
				...
				if (player.getClubDB() != null) {
					desc += "Eingetragen in Club: " + player.getClubDB().getInfoStringDB();
					if (player.isMarked()) {
						desc += " (✗)";
					...
					desc += "Eingetragen in Club: ---\n";
				}
				if (player.getClubAPI() != null) {
					desc += "Ingame in Club: " + player.getClubAPI().getInfoStringAPI() + "\n";
				} else {
					desc += "Ingame in Club: ---\n";
				}
	...
	public void onCommandAutoCompleteInteraction(@Nonnull CommandAutoCompleteInteractionEvent event) {
		...
		if (focused.equals("player")) {
			List<Command.Choice> choices = DBManager.getPlayerlistAutocomplete(input, DBManager.InClubType.ALL);
