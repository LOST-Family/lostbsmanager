...
import webserver.api.dto.ClubDTO;
...
/**
 * REST API Server (ported from lostmanager) Club-specific endpoints removed.
 */
public class RestApiServer {
	...
	public RestApiServer(int port) {
		...
		if (this.apiToken == null || this.apiToken.isEmpty()) {
			System.err.println(
					"WARNING: lostbsmanager_API_SECRET is not set. API endpoints will be accessible without authentication.");
		}
	}
	...
	public void start() throws IOException {
		...
		server.createContext("/api/Clubs/", new ClubSpecificHandler());
	...
	/**
	 * Handler for Club-specific endpoints:
	 * - GET /api/Clubs/{tag} - Club info
	 * - GET /api/Clubs/{tag}/members - Club members (DB only)
	 */
	private class ClubSpecificHandler implements HttpHandler {
		@Override
		public void handle(HttpExchange exchange) throws IOException {
			if ("OPTIONS".equals(exchange.getRequestMethod())) {
				addCorsHeaders(exchange);
			...
			try {
				...
				String ClubTag = parts[3];

				// Validate Club exists (DB only)
				Club Club = new Club(ClubTag);
				if (!Club.ExistsDB()) {
					sendResponse(exchange, 404, "{\"error\":\"Club not found\"}");
					return;
				}
				...
				if (parts.length >= 5) {
					...
					if ("members".equals(subPath)) {
						handleClubMembers(exchange, Club);
						return;
					} else {
				...
				handleException(exchange, "ClubSpecificHandler", e);
			}
		}

		private void handleClubMembers(HttpExchange exchange, Club Club) throws Exception {
			ArrayList<Player> members = Club.getPlayersDB();
	...
	/**
	 * Handler for GET /api/Clubs Returns all available Clubs
	 */
	private class ClubsHandler implements HttpHandler {
		@Override
		public void handle(HttpExchange exchange) throws IOException {
			if ("OPTIONS".equals(exchange.getRequestMethod())) {
				addCorsHeaders(exchange);
			...
			try {
				// Get all Club tags from database
				ArrayList<String> ClubTags = DBManager.getAllClubs();

				// Convert to DTOs
				List<ClubDTO> Clubs = new ArrayList<>();
				for (String tag : ClubTags) {
					try {
						Club Club = new Club(tag);
						Clubs.add(new ClubDTO(Club));
					} catch (Exception e) {
						System.err.println("Error processing Club " + tag + ": " + e.getMessage());
					}
				}
				...
				handleException(exchange, "ClubsHandler", e);
			}
		}
	}
	 ...
	 * Club,
	 * along with their highest level linked account that is currently in a Club.
	 */
		...
		public void handle(HttpExchange exchange) throws IOException {
			...
			try {
				...
				for (Member member : distinctMembers) {
					...
					// Filter: must be in a Club (getClubDB() != null) and find highest level
					for (Player p : linkedAccounts) {
						// Only consider accounts that are actually in a Club (per DB)
						if (p.getClubDB() != null) {
