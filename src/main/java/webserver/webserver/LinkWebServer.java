...
public class LinkWebServer {
	...
	public static void start() {
		synchronized (lock) {
			...
			apiSecret = System.getenv("lostbsmanager_API_SECRET");
			String portEnv = System.getenv("lostbsmanager_PORT");
			...
			if (apiSecret == null || apiSecret.isEmpty()) {
				System.out.println("[LinkAPI] Warning: lostbsmanager_API_SECRET not set. API authentication disabled.");
			} else {
			...
			try {
				...
				System.err.println("  2. Try a different port: export lostbsmanager_PORT=8080");
	...
	private static class HealthCheckHandler implements HttpHandler {
		...
		public void handle(HttpExchange exchange) throws IOException {
			...
			try {
				...
				response.put("service", "lostbsmanager");
		...
		public void handle(HttpExchange exchange) throws IOException {
			...
			try {
				...
				try {
					...
					DBUtil.executeUpdate("INSERT INTO players (bs_tag, discord_id, name) VALUES (?, ?, ?)", finalTag,
					...
					if (p.getClubDB() != null) {
						System.out.println("[LinkAPI] Player " + p.getInfoStringAPI()
								+ " already in a Club. Cannot be added to Waitlist (again)" + " (source: " + finalSource
								+ ")");
						response.put("waitlistsuccess", false);
					} else {
						// Add to waitlist
						DBUtil.executeUpdate(
								"INSERT INTO Club_members (player_tag, Club_tag, Club_role) VALUES (?, ?, ?)", finalTag,
								"warteliste", "member");
						System.out.println("[LinkAPI] Player " + p.getInfoStringAPI()
					...
					try (java.sql.ResultSet rs = pstmt.executeQuery()) {
						while (rs.next() && results.length() < 25) {
							...
							if (display.toLowerCase().contains(query)
								...
								entry.put("Club", ClubName != null ? ClubName : "");
	 ...
	 * Removes a player's Club membership and link.
	 */
	private static class RemovePlayerHandler implements HttpHandler {
		...
		public void handle(HttpExchange exchange) throws IOException {
			...
			try {
				...
				datautil.DBUtil.executeUpdate("DELETE FROM Club_members WHERE player_tag = ?", finalTag);

				// Remove player link
				datautil.DBUtil.executeUpdate("DELETE FROM players WHERE bs_tag = ?", finalTag);
